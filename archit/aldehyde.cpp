...
vector<int> FindParentPrefix(vector<Pair<vector<int>, string>> LocantPairs){
    vector<int> FoundNumbers;
    //find the prefixes, i.e. the meth, eth, ....
    for (int i = 0; i < LocantPairs.size(); ++i ){
        string temp = LocantPairs[i].Data.Value;
        for (int j = 0; j < NumberPrefixes.size(); ++j){
            string temp2 = temp;
            FindAndReplaceAll(temp, NumberPrefixes[j].Data.Key, ":");
            if (temp2!=temp){
                //found
                FoundNumbers.push_back(NumberPrefixes[j].Data.Value);
            };
        };
    };
    return FoundNumbers;
...
vector<GeneralObject> ConvertStringNamesIntoNumberOfCarbonAtomsAndCreateWriteData(vector<int> ParentNums, vector<Pair<vector<int>, string>> LocantPairs) {
    ...
    int CurrentParentNumsPointer = 0;
    for (size_t i = 0; i < LocantPairs.size(); ++i) {
        ...
        if (val.find("ene") != string::npos || val.find("en") != string::npos) {
            ...
            for (int pos : LocantPairs[i].Data.Key) {
                ...
                go.JSON.Atoms = to_string(ParentNums[CurrentParentNumsPointer]);
                FinalVector.push_back(go);
            }
            ++CurrentParentNumsPointer;
        }
    }
...
bool endsWithNChars(const std::string& mainStr, const std::string& subStr, size_t n) {
    if (mainStr.length() >= n && subStr.length() == n) {
        return mainStr.substr(mainStr.length() - n) == subStr;
    }
    return false;
}
...
void AldehydeParser(string Name){
    ...
    if (Name == "methanol-") {
        // Convert methanol to systematic name methan-1-ol
        Name = "methan-1-ol-";
    }
    else if (Name == "ethanol-") {
        // Convert ethanol to systematic name ethan-1-ol  
        Name = "ethan-1-ol-";
    }
    else if (Name == "propanol-") {
        // Convert propanol to systematic name propan-1-ol
        Name = "propan-1-ol-";
    }
    
    //special exceptions for the methane molecules. METHANE, not any methanol or anything else. that needs additional considerations. Loop 1.
    if (endsWithNChars(Name, "methane-", 8)){
        //for that, we need to make a separate GeneralObject, and remove the last GeneralObject in finalobject.
    ...
    else if(endsWithNChars(Name, "methan-1-one-", 13)){
    vector<string> BrokenDown = BreakDownString(Name);
    vector<Pair<string, string>> Locants = ConvertFromRawLocantToRawPair(BrokenDown);
    vector<Pair<vector<int>, string>> RLocants = ConvertFromRawLocantPairToLocantPair(Locants);
    vector<int> Numbers = FindParentPrefix(RLocants);
    ...
    else if(endsWithNChars(Name, "methan-1-al-", 12)){
    vector<string> BrokenDown = BreakDownString(Name);
    vector<Pair<string, string>> Locants = ConvertFromRawLocantToRawPair(BrokenDown);
    vector<Pair<vector<int>, string>> RLocants = ConvertFromRawLocantPairToLocantPair(Locants);
    vector<int> Numbers = FindParentPrefix(RLocants);
    ...
    FinalObjects[FinalObjects.size()-1].JSON.Atoms = "1";
    WriteDataToFile(FinalObjects);
    }//all others molecules in below loop
